...
メンバーの削除を行う場合は、そのメンバーのアイコン画像がもう参照されていないことを必ず確認した上で、そのアイコン画像も `static/images/members/` から削除してください。

また、`gearsAndSettings` フィールドに指定されている `GEARS_AND_SETTINGS` 定数の要素の削除については、必ずユーザーに確認をとってください。
その際、その要素の `avatar` フィールドで指定されている等身イラストのファイル（`static/images/members/characters/` 配下）の削除も合わせて確認してください。
