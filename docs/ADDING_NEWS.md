...
1. `pnpm run article` を実行。
1. 生成されたファイルを編集。
1. [サムネイル画像の設定方法](#サムネイル画像の設定方法)に従ってサムネイル画像を設定。(任意)

記事 ID は省略可能な引数で指定できます。省略時は実行日の `YYYYMMDD` を使います。

```bash
pnpm run article
pnpm run article 20230413
pnpm run article 2023041303
pnpm run article 20230413_text
```

生成先は ID の日付に対応する `/articles/{YYYY}/{M}/{記事ID}.md` です（月はゼロ埋めなし）。

- 日付だけの ID は、同名ファイルがある場合に `02` から順に加算し、競合しない番号を使います。ID 省略時も同様です。
- ナンバリングまたは `_text` を指定した場合は、その ID をそのまま使います。同名ファイルがある場合はメッセージ付きでエラーになり、上書きや自動採番は行いません。
- 不正な形式・日付・ナンバリングはエラーになります。
